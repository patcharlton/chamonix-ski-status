...
function getOverallCondition(weather: WeatherStation[]): { emoji: string; label: string; color: string } {
  ...
  if (maxWind > 50 || avalancheRisk >= 4) {
    return { emoji: "⚠️", label: "Challenging", color: "bg-amber-100 text-amber-800" };
  }
  if (snowQuality === "FRAICHE" && freshSnow > 20) {
    return { emoji: "🎿", label: "Powder Day", color: "bg-blue-100 text-blue-800" };
  }
  if (avgTemp > 5) {
    return { emoji: "☀️", label: "Spring Skiing", color: "bg-yellow-100 text-yellow-800" };
  }
  if (avgTemp < -10) {
    return { emoji: "❄️", label: "Cold & Crisp", color: "bg-cyan-100 text-cyan-800" };
  }
  if (maxWind > 35) {
    return { emoji: "💨", label: "Windy", color: "bg-gray-100 text-gray-800" };
  }
  return { emoji: "✨", label: "Great Conditions", color: "bg-green-100 text-green-800" };
}
